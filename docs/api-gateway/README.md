...
iac/modules/
├── api-gateway/    REST API, stage, deployment, usage plan, API key, logs, CORS de erro
└── vpc-link/       NLB interno, target group, ASG attachment, VPC Link

scripts/validate-openapi.sh             valida o contrato antes do apply
```
...
O Terraform resolve quatro placeholders no contrato durante o apply:
...
terraform output -raw api_key_value       # chave para o header x-api-key
...
A API key é gerada a cada criação do ambiente e **não é versionada**
([ADR-006](../adr/ADR-006-ambientes-efemeros-e-conta-educacional.md)).
Para adicionar um segundo consumidor, basta mais um par
`aws_api_gateway_api_key` + `aws_api_gateway_usage_plan_key`.
...
**Não há authorizer no gateway.** O JWT é emitido pela Lambda em
...
|---|---|---|
| `403 Forbidden` sem corpo | falta `x-api-key` ou chave desvinculada do plano | `terraform output -raw api_key_value` |
